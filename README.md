...
特别的，如果你想知道课程表文件的介绍、使用的方法、最新的版本与更新等信息，请前往 [封面](<./封面.md>) 页面~
...
- 先在`cmd`中执行`python -m http.server 8000`
- 运行`2_html转pdf.py`，会将`html`文件夹中的`html`文件转换为`pdf`，保存在`pdf`文件夹
- 如果存在某个`html`文件对应的`pdf`文件，会跳过

### 2.pdf清洗

- 运行`2_pdf清洗.py`，会将`pdf`文件夹中的`pdf`文件进行清洗，将上下过多的留白进行去除，保存在`clean_pdf`文件夹

### 3.合并pdf

- 运行`3_合并.py`会将`clean_pdf`文件夹中的`pdf`合并为一个`pdf`，保存到`成信大课程表.pdf`中。同时会添加年级信息在右上角。
...
- guide.cuit.dev 公众群：[973721542](<https://qm.qq.com/q/PCGDMkbGc6>)
- 阿发的小群：[736723590](<https://qm.qq.com/q/cv9tlkrPJC>)
- 2026 CUIT学生数学建模协会新生群：[1037962539](<https://qm.qq.com/q/PP6oSoaVie>)
- 2026学生数学建模协会龙泉校区新生群：[1036801771](<https://qm.qq.com/q/eyAmxmYRvG>)
